...
def xy2anchors(x,y,imgwidth,scale_factor):
    xa,ya=intr(x/scale_factor),intr(y/scale_factor)
...
def _vectorize_boat(b,scale_factor):
    h=b['hull']
    imgwidth=b['meta']['imgwidth']
    #kprint(h,r=1)
    w=b['wake']
    vb={}
    v=[]
    ...
    if 'xcenter' in h and 'ycenter' in h:
        ...
        x,y=0,0
    xa,ya,dx,dy=xy2anchors(x,y,imgwidth,scale_factor)
    v=[dx,dy]+v
    vb['hull']=dict(xy=(xa,ya),v=v)
...
scale_factor=10
A=zeros((imgwidth//scale_factor,imgwidth//scale_factor,nsteps*6))

for b in boats_through_time:
    k='gt'
    for i in rlen(b[k]):
        vb=vectorize_boat(b[k][i],scale_factor)
        for l in vb:
            x,y=vb[l]['xy']
            v=vb[l]['v']
